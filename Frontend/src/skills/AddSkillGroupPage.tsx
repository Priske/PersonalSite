...
export function AddSkillGroupPage() {
  ...
  const highestDisplayOrder = useMemo(() => {
    ...
    return groups.length === 0
      ? 0
      : Math.max(...groups.map((group) => group.displayOrder));
  }, [groupsQuery.data]);
  ...
  const isSaving = createSkillGroupMutation.isPending;
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!cleanedName) {
      setValidationMessage("The skill group name is required.");
  ...
  return (
    <section className="manage-skill-group-page">
      <header className="manage-skill-group-page__header">
        <div>
          <p className="manage-skill-group-page__eyebrow">Create skill group</p>

          <h2>{name.trim() || "New skill group"}</h2>

          <p>
            Create a new skill group. Skills can be added after the group has
            been created.
          </p>
        </div>
      ...
      <form className="manage-skill-group-form" onSubmit={handleSubmit}>
        ...
        {createSkillGroupMutation.isError && (
          <p className="form-message form-message--error"></p>
        )}
        ...
        <div className="manage-skill-group-form__actions">
          <button
            ...
            disabled={isSaving}
          >
            {isSaving ? "Creating..." : "Create skill group"}
          </button>
