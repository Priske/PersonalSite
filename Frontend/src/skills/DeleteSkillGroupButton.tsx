...
export function DeleteSkillGroupButton({
  ...
  function leaveDeletedSkillGroup() {
    ...
    queryClient.removeQueries({
      queryKey: ["skill-groups", "detail", skillGroupId],
      exact: true,
    });

    queryClient.removeQueries({
      queryKey: ["skill-groups", skillGroupId, "skills"],
      exact: true,
    });
  ...
  const deleteMutation = useMutation({
    mutationFn: () => deleteSkillGroup(skillGroupId),
  ...
  return (
    ...
    >
      <div className="delete-skill-group-confirmation__content">
        ...
        <h3 id="delete-skill-group-heading">Delete this skill group?</h3>

        <p className="delete-skill-group-confirmation__description">
          This action cannot be undone. The skill group{" "}
          <strong>{skillGroupName}</strong> and all of its attached skills will
      ...
      >
        ...
        {mutationStatus === 403 && (
          <p className="form-message form-message--error">
            You are not allowed to delete this skill group.
          </p>
        )}
        ...
        {mutationStatus === 404 && (
          <div className="delete-skill-group-confirmation__missing">
            <p className="form-message form-message--error">
              This skill group no longer exists. It may already have been
              deleted.
            </p>
